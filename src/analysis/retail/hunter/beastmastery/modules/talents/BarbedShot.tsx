...
class BarbedShot extends Analyzer {
  ...
  frenzyBuffDuration: number = this.selectedCombatant.hasTalent(
    TALENTS.SAVAGERY_BEAST_MASTERY_TALENT,
  )
    ? SAVAGERY_FRENZY_DURATION
    : ORIGINAL_FRENZY_DURATION;
  ...
  onCast(event: CastEvent) {
    ...
    if (this.selectedCombatant.hasTalent(TALENTS.SAVAGERY_BEAST_MASTERY_TALENT)) {
      const tooltip = (
        <>
          {baseTooltip}
          You used <SpellLink spell={TALENTS.BARBED_SHOT_TALENT} /> while talented into{' '}
          <SpellLink spell={TALENTS.SAVAGERY_BEAST_MASTERY_TALENT} /> giving your{' '}
